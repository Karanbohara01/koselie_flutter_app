...
class CreatePostsParams extends Equatable {
  ...
  const CreatePostsParams.empty()
      : caption = '',
        ...
        price = '',
        image = null,
        category = const CategoryEntity.empty(); // Use an empty category method
...
class CreatePostsUseCase implements UsecaseWithParams<void, CreatePostsParams> {
  ...
  CreatePostsUseCase({required this.postsRepository});

  @override
  Future<Either<Failure, void>> call(CreatePostsParams params) async {
    // Make sure to handle the case when image is null by passing an empty string or default value
    final postEntity = PostsEntity(
      caption: params.caption,
      description: params.description,
      price: params.price,
      location: params.location,
      image: params.image ?? '', // Default to empty string if image is null
      category: params.category,
    );

    return await postsRepository.createPost(postEntity);
  }
}
